...
define('buzzcrud_PATH', str_replace('\\', '/', dirname(__file__))); // str_replace - windows trick
require (buzzcrud_PATH . '/buzzcrud_config.php'); // configuration
require (buzzcrud_PATH . '/buzzcrud_db.php'); // database class
mb_internal_encoding(buzzcrud_config::$mbencoding); // sets multibyte encoding globaly
date_default_timezone_set(@date_default_timezone_get()); // buzzcrud code not depends on timezone, but this can fix some warnings
...
class buzzcrud
{
    ...
    protected $load_view = array(
        'list' => 'buzzcrud_list_view.php',
        'create' => 'buzzcrud_detail_view.php',
        'edit' => 'buzzcrud_detail_view.php',
        'view' => 'buzzcrud_detail_view.php');
    ...
    /** constructor, sets basic buzzcrud vars (they can be changed by public pethods) */
    protected function __construct()
    {
        buzzcrud_config::$scripts_url = self::check_url(buzzcrud_config::$scripts_url, true);
        buzzcrud_config::$editor_url = self::check_url(buzzcrud_config::$editor_url);
        buzzcrud_config::$editor_init_url = self::check_url(buzzcrud_config::$editor_init_url);

        $this->limit = buzzcrud_config::$limit;
        $this->limit_list = buzzcrud_config::$limit_list;
        $this->column_cut = buzzcrud_config::$column_cut;
        $this->show_primary_ai_field = buzzcrud_config::$show_primary_ai_field;
        $this->show_primary_ai_column = buzzcrud_config::$show_primary_ai_column;

        $this->benchmark = buzzcrud_config::$benchmark;
        $this->start_minimized = buzzcrud_config::$start_minimized;
        $this->remove_confirm = buzzcrud_config::$remove_confirm;
        $this->upload_folder_def = buzzcrud_config::$upload_folder_def;

        $this->theme = buzzcrud_config::$theme;
        $this->is_print = buzzcrud_config::$enable_printout;
        $this->is_title = buzzcrud_config::$enable_table_title;
        $this->is_csv = buzzcrud_config::$enable_csv_export;
        $this->is_numbers = buzzcrud_config::$enable_numbers;
        $this->is_pagination = buzzcrud_config::$enable_pagination;
        $this->is_search = buzzcrud_config::$enable_search;
        $this->is_limitlist = buzzcrud_config::$enable_limitlist;
        $this->is_sortable = buzzcrud_config::$enable_sorting;

        $this->language = buzzcrud_config::$language;

        $this->search_pattern = buzzcrud_config::$search_pattern;

        $this->demo_mode = buzzcrud_config::$demo_mode;

        $this->default_tab = buzzcrud_config::$default_tab;
        $this->prefix = buzzcrud_config::$dbprefix;
        $this->is_rtl = buzzcrud_config::$is_rtl;

        $this->strip_tags = buzzcrud_config::$strip_tags;
        $this->safe_output = buzzcrud_config::$safe_output;
    }
    ...
    {
        if (isset($_POST['buzzcrud']['instance']) && isset($_POST['buzzcrud']['key']) && isset($_POST['buzzcrud']['task']))
        {
            self::init_prepare('post');
            $key = $_POST['buzzcrud']['key'] ? $_POST['buzzcrud']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_POST['buzzcrud']['instance'] ? $_POST['buzzcrud']['instance'] : self::error('Instance name cannot be empty');
            $is_get = false;
        }
        elseif (isset($_GET['buzzcrud']['instance']) && isset($_GET['buzzcrud']['key']) && isset($_GET['buzzcrud']['task']) && $_GET['buzzcrud']['task'] ==
            'file')
        {
            self::init_prepare('get');
            $key = $_GET['buzzcrud']['key'] ? $_GET['buzzcrud']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_GET['buzzcrud']['instance'] ? $_GET['buzzcrud']['instance'] : self::error('Instance name cannot be empty');
            $is_get = true;
        }
        else
        {
        ...
        if (isset($_SESSION['buzzcrud_session'][$inst_name]['key']) && $_SESSION['buzzcrud_session'][$inst_name]['key'] == $key)
        {
    ...
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
                $sess_name = (buzzcrud_config::$dynamic_session && isset($_POST['buzzcrud']['sess_name']) && $_POST['buzzcrud']['sess_name']) ? $_POST['buzzcrud']['sess_name'] :
                    buzzcrud_config::$sess_name;
                break;
            case 'get':
                $sess_name = (buzzcrud_config::$dynamic_session && isset($_GET['buzzcrud']['sess_name']) && $_GET['buzzcrud']['sess_name']) ? $_GET['buzzcrud']['sess_name'] :
                    buzzcrud_config::$sess_name;
                break;
            default:
                $sess_name = buzzcrud_config::$sess_name;
                break;
        }
        self::session_start($sess_name);
        if (is_callable(buzzcrud_config::$before_construct))
        {
            call_user_func(buzzcrud_config::$before_construct);
        }
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !buzzcrud_config::$external_session && !buzzcrud_config::$alt_session)
        {
            if (!headers_sent())
            {
                session_name($sess_name);
                session_cache_expire(buzzcrud_config::$sess_expire);
                session_set_cookie_params(0, '/');
                session_start();
            }
            else
                self::error('buzzcrud can not create session, because the output is already sent into browser. 
                Try to define buzzcrud instance before the output start or use session_start() at the beginning of your script');
        }
        if (buzzcrud_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, $this->sess_id, time() + buzzcrud_config::$alt_lifetime * 60, '/');
            }
            else
                self::error('buzzcrud can not start session, because the output is already sent into browser. 
                Try to define buzzcrud instance before the output start or use <strong>buzzcrud::session_start();</strong> at the beginning of your script');
        }
    }
    ...
    public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
    {
        if ($instance_name && $field && $inner_tbl && $tbl_field)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                $instance = buzzcrud::get_instance($instance_name); // just another buzzcrud object
                ...
                switch ($type)
                {
                    ...
                    case 'point':
                        ...
                        $def_attr = array( // defaults
                            'text' => buzzcrud_config::$default_text,
                            'search_text' => buzzcrud_config::$default_search_text,
                            'zoom' => buzzcrud_config::$default_zoom,
                            'width' => buzzcrud_config::$default_width,
                            'height' => buzzcrud_config::$default_height,
                            'search' => buzzcrud_config::$default_coord,
                            'coords' => buzzcrud_config::$default_search);
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, $attr);
                        break;
                    case 'remote_image':
    ...
    protected function _run_task()
    {
        ...
        switch ($this->task)
        {
            ...
            case 'print':
                ...
                $this->_set_field_types('list', buzzcrud_config::$print_all_fields);
                $this->theme = 'printout';
                return $this->_list();
                break;
            case 'depend':
            ...
            case 'file':
                ...
                break;
            case 'csv':
                $this->_set_field_types('list', buzzcrud_config::$csv_all_fields);
                return $this->_csv();
                break;
            case 'list':
            default:
    ...
    protected function render_custom_datagrid()
    {
        ...
        $db = buzzcrud_db::get_instance($this->connection);
    ...
    protected function render_custom_csv()
    {
        ...
        fputcsv($output, $this->columns_names, buzzcrud_config::$csv_delimiter, buzzcrud_config::$csv_enclosure);
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process, saves memory
        {
            ...
            fputcsv($output, $out, buzzcrud_config::$csv_delimiter, buzzcrud_config::$csv_enclosure);
        }
    }
    ...
    protected function render_output()
    {
        ...
        {
            $contents = '';
            if (!self::$css_loaded && !buzzcrud_config::$manual_load)
            {
                $contents .= self::load_css();
            }
            ob_start();
            include (buzzcrud_PATH . '/' . buzzcrud_config::$themes_path . '/' . $this->theme . '/buzzcrud_container.php');
            $contents .= ob_get_contents();
            ob_end_clean();
            unset($this->data);
            if (!self::$js_loaded && !buzzcrud_config::$manual_load)
            {
    ...
    protected function after_render()
    {
        ...
        if (is_callable(buzzcrud_config::$after_render))
        {
            call_user_func(buzzcrud_config::$after_render);
        }
    }
    ...
    {
        if (isset($_POST['buzzcrud'][$field]))
        {
            if (buzzcrud_config::$auto_xss_filtering)
            {
            ...
            if (($field == 'postdata' or $field == 'unique') && $_POST['buzzcrud'][$field])
            {
                $data_keys = array_keys($_POST['buzzcrud'][$field]);
                foreach ($data_keys as $k => $key)
                {
                    $data_keys[$k] = $xss ? $xss->xss_clean($this->fieldname_decode($key)) : $this->fieldname_decode($key);
                    if ($xss)
                    {
                        $_POST['buzzcrud'][$field][$key] = $xss->xss_clean($_POST['buzzcrud'][$field][$key]);
                    }
                }
                return array_combine($data_keys, $_POST['buzzcrud'][$field]);
            }
            elseif ($filter)
            {
                switch ($filter)
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_POST['buzzcrud'][$field]) : $_POST['buzzcrud'][$field]);
                        break;
                    case 'int':
                        return (int)$_POST['buzzcrud'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_POST['buzzcrud'][$field]) : $_POST['buzzcrud'][$field]);
                        break;
                    case 'base64':
                        return $xss ? $xss->xss_clean($this->fieldname_decode($_POST['buzzcrud'][$field])) : $this->fieldname_decode($_POST['buzzcrud'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_POST['buzzcrud'][$field]) : $_POST['buzzcrud'][$field];
                        break;
                }
            }
            else
    ...
    protected function _get($field = '', $default = false, $filter = false)
    {
        if (isset($_GET['buzzcrud'][$field]))
        {
            ...
            if ($filter)
            {
                ...
                {
                    case 'key':
                        return str_replace('`', '', $xss ? $xss->xss_clean($_GET['buzzcrud'][$field]) : $_GET['buzzcrud'][$field]);
                        break;
                    case 'int':
                        return (int)$_GET['buzzcrud'][$field];
                        break;
                    case 'trim':
                        return trim($xss ? $xss->xss_clean($_GET['buzzcrud'][$field]) : $_GET['buzzcrud'][$field]);
                        break;
                    default:
                        return $xss ? $xss->xss_clean($_GET['buzzcrud'][$field]) : $_GET['buzzcrud'][$field];
                        break;
                }
            }
            else
        ...
        if (!$this->primary_val)
        {
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (buzzcrud_config::$email_enable_html)
                        $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="' . $alert['link'] .
                            '" target="_blank">' . $alert['link'] . '</a>' : '');
                    else
                        $message = $alert['message'] . "\r\n\r\n" . ($alert['link'] ? $alert['link'] : '');
                    $this->send_email($postdata[$alert['column']], $alert['subject'], $message, $alert['cc'], buzzcrud_config::$email_enable_html);
                }
            }
            if ($this->mass_alert_create)
            {
                foreach ($this->mass_alert_create as $alert)
                {
                    ...
                    {
                        $this->send_email($row[$alert['email_column']], $alert['subject'], $message, array(), buzzcrud_config::$email_enable_html);
                    }
                }
            }
            $pd = new buzzcrud_postdata($postdata, $this);
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        else
        {
            ...
            $this->limit = $this->_post('limit', ($this->limit ? $this->limit : buzzcrud_config::$limit));
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type)
        {
            ...
            case 'bool':
            case 'boolean':
                if ($max_l == 1 && buzzcrud_config::$make_checkbox)
                {
            ...
            case 'mediumtext':
            case 'longtext':
                if (!isset($this->no_editor[$field_index]) && buzzcrud_config::$auto_editor_insertion)
            ...
            case 'year':
                ...
                break;
            case 'enum':
                $this->field_type[$field_index] = buzzcrud_config::$enum_as_radio ? 'radio' : 'select';
                $this->field_attr[$field_index]['values'] = $max_l;
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = $row['Default'];
                break;
            case 'set':
                $this->field_type[$field_index] = buzzcrud_config::$set_as_checkboxes ? 'checkboxes' : 'multiselect';
                $this->field_attr[$field_index]['values'] = $max_l;
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = $row['Default'];
                break;
            case 'point':
                ...
                if (!isset($this->defaults[$field_index]))
                    $this->defaults[$field_index] = buzzcrud_config::$default_point ? buzzcrud_config::$default_point : '0,0';
                break;
        }
    }
    ...
    protected function _render_list()
    {
        ...
        if ($this->column === false)
        {
            ...
            elseif (!buzzcrud_config::$search_all)
            {
                if ($this->search_columns)
                {
        ...
        $mode = 'list';
        $view_file = buzzcrud_PATH . '/' . buzzcrud_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    protected function _render_details($mode)
    {
        ...
        */
        ...
        if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit')) // restoring nested objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    ...
                    if ($mode == 'view' && buzzcrud_config::$nested_readonly_on_view)
                    {
                    ...
                    $this->nested_rendered[$inst_name] =
                        '<div class="buzzcrud-nested-container buzzcrud-container"><div class="buzzcrud-ajax" id="buzzcrud-ajax-' . base_convert(rand(), 10,
                        36) . '">' . $instance->render('list') . '</div></div>';
                }
            }
        }

        $view_file = buzzcrud_PATH . '/' . buzzcrud_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
        ...
        //$this->data .= '<div class="buzzcrud-nested-container buzzcrud-container"><div class="buzzcrud-ajax" id="buzzcrud-ajax-' .
        //    base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        $this->nested_rendered[$inst_name] = '<div class="buzzcrud-nested-container buzzcrud-container"><div class="buzzcrud-ajax" id="buzzcrud-ajax-' .
        base_convert(rand(), 10, 36) . '">' . $instance->render('list') . '</div></div>';
        }
        }
        }*/
    ...
    protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
    {
        if ($total > $limit)
        {
            ...
            '"><a href="javascript:;" class="buzzcrud-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
            }
            }*/
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, buzzcrud_config::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, buzzcrud_config::$mbencoding);
        if ($slen <= $len)
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        ...
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, buzzcrud_config::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, buzzcrud_config::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, buzzcrud_config::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, buzzcrud_config::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        {
            if (isset($_SESSION['buzzcrud_session']) && $_SESSION['buzzcrud_session'])
            {
                foreach ($_SESSION['buzzcrud_session'] as $s_key => $s_val)
                { // workaround on some servers session duplication
                    $old_time = isset($s_val['time']) ? (int)$s_val['time'] : 0;
                    if ($time > $old_time + buzzcrud_config::$autoclean_timeout) // autocleaner

                        unset($_SESSION['buzzcrud_session'][$s_key]);
                }
            }
        }
        ...
        {
            $_SESSION['buzzcrud_session'][$inst_name][$item] = $this->{$item};
        }
        $_SESSION['buzzcrud_session'][$inst_name]['before'] = $this->find_prev_task();

        if (buzzcrud_config::$alt_session)
        {
            $data = $this->encrypt($_SESSION['buzzcrud_session']);

            if (class_exists('Memcache'))
            {
                $mc = new Memcache();
                $mc->connect(buzzcrud_config::$mc_host, buzzcrud_config::$mc_port);
                $res = $mc->set($this->sess_id, $data, false, buzzcrud_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(buzzcrud_config::$mc_host, buzzcrud_config::$mc_port);
                $res = $mc->set($this->sess_id, $data, buzzcrud_config::$alt_lifetime * 60);
            }
            else
            {
                self::error('Can\'t use alternative session. Memcache(d) is not available');
            }
            unset($_SESSION['buzzcrud_session']);
            if (!$res)
            {
    ...
    public function import_vars($key = false)
    {
        if (buzzcrud_config::$alt_session)
        {
            ...
            $_SESSION['buzzcrud_session'] = $this->decrypt($data[0], $data[1]);
            unset($data);
            if (!$_SESSION['buzzcrud_session'])
            {
        ...
        foreach ($this->params2save() as $item)
        {
            $this->{$item} = $_SESSION['buzzcrud_session'][$inst_name][$item];
        }
    ...
    {
        $tag = array('class' => 'buzzcrud-input');
    ...
    {
        return '<span class="buzzcrud-none">' . $value . '</span>';
    }
    ...
    {
        if (buzzcrud_config::$clickable_list_links)
        {
    ...
    protected function create_radio($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'buzzcrud-radio-label');
    ...
    protected function create_checkboxes($name, $value = '', $tag = array())
    {
        ...
        $label_tag = array('tag' => 'label', 'class' => 'buzzcrud-checkboxes-label');
    ...
    protected function create_hidden($name, $value = '', $tag = array())
    {
        return $this->single_tag($tag + array(
            ...
            'type' => 'hidden',
            'value' => $value,
            'name' => $name), 'buzzcrud-input', $this->field_attr[$name], true);
    }
    ...
    protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
    {
        ...
        {
            $options = false;
            if (buzzcrud_config::$lists_null_opt)
            {
                foreach ($values as $val)
                {
    ...
    protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out .= $this->open_tag('div', 'buzzcrud-upload-container'); // file and delete button
        if ($value)
        {
            $out .= $this->open_tag('span', 'buzzcrud-file-name');
            ...
            $attr = array(
                ...
                'class' => 'buzzcrud-file buzzcrud-' . $ext,
                'target' => '_blank');
        ...
        {
            $out .= $this->open_tag('span', 'buzzcrud-nofile');
        ...
        $attr = array(
            ...
            'value' => $value,
            'type' => 'hidden');
        $out .= $this->single_tag('input', 'buzzcrud-input', $attr, true);
        // upload button
    ...
    {
        $out = '';
        $out .= $this->open_tag('span', $this->theme_config('upload_button'), array('class' => 'buzzcrud-add-file'));
        if (!$this->is_rtl)
        {
        ...
        $attr = array(
            ...
            'data-type' => 'file',
            'data-field' => $name,
            'class' => 'buzzcrud-upload',
            'name' => 'buzzcrud-attach');
    ...
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'buzzcrud-upload-container'); // image and delete button
        if ($value)
        {
            $binary = isset($this->upload_config[$name]['blob']) ? true : false;
            if ($binary && !$is_upload)
        ...
        {
            $out .= $this->open_tag('span', 'buzzcrud-noimage');
    ...
    protected function create_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = buzzcrud_config::$default_point ? buzzcrud_config::$default_point : '0,0';
        }
    ...
    protected function create_view_point($name, $value = '', $tag = array())
    {
        ...
        if (!$value)
        {
            $value = buzzcrud_config::$default_point;
        }
        if ($value)
        {
            $tag = array(
    ...
    {
        exit('<div class="buzzcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">buzzcrud</span>' . $text . '</div>');
    }
    ...
    protected function _upload_file()
    {
        ...
        if (isset($_FILES) && isset($_FILES['buzzcrud-attach']) && !$_FILES['buzzcrud-attach']['error'])
        {
    ...
    {
        $out = ''; // upload container
        $out .= $this->open_tag('div', 'buzzcrud-upload-container');
        ...
        $attr = array(
            ...
            'alt' => '');
        $out .= $this->single_tag('img', 'buzzcrud-crop', $attr);
    ...
    protected function _compare($val1, $operator, $val2)
    {
        switch ($operator)
        {
            ...
            case '!=':
                return ($val1 != $val2) ? true : false;
            case '^=':
                return (mb_strpos($val1, $val2, 0, buzzcrud_config::$mbencoding) === 0) ? true : false;
            case '$=':
                return (mb_strpos($val1, $val2, 0, buzzcrud_config::$mbencoding) == (mb_strlen($val1, buzzcrud_config::$mbencoding) -
                    mb_strlen($val2, buzzcrud_config::$mbencoding))) ? true : false;
            case '~=':
                return (mb_strpos($val1, $val2, 0, buzzcrud_config::$mbencoding) !== false) ? true : false;
            default:
                return false;
        }
    }
    protected function create_modal($field, $content, $image = false)
    {
        ...
        $out .= $this->open_tag('a', 'buzzcrud_modal', $attr);
        if (buzzcrud_config::$images_in_grid && $image)
        {
            $out .= $content;
        }
        else
        ...
        if (isset($this->field_type[$field]))
        {
            switch ($this->field_type[$field])
            {
                ...
                case 'image':
                    if ($value)
                    {
                        if (buzzcrud_config::$images_in_grid)
                        {
                            $settings = $this->upload_config[$field];
                            ...
                            $out .= $this->single_tag('img', '', array(
                                ...
                                'style' => 'max-height: ' . buzzcrud_config::$images_in_grid_height . 'px;'));
                        }
                        else
                        {
                            $out .= $this->open_tag('a', '', array('target' => '_blank', 'href' => $this->file_link($field, $primary_val)));
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        if ($this->buttons)
        {
            foreach ($this->buttons as $button)
            {
                if ($this->is_button($button['name'], $row))
                {
                    ...
                    if (buzzcrud_config::$button_labels)
                    {
        ...
        if (!isset($this->hide_button['duplicate']) && !$this->table_ro && $this->is_duplicate($row))
        {
            $tag = array(
                'tag' => 'a',
                'class' => 'buzzcrud-action ' . $this->theme_config('grid_default'),
    ...
    public static function check_url($url, $scr_url = false)
    {
        ...
        elseif (buzzcrud_config::$urls2abs)
        {
    ...
    protected function file_link($field, $primary_val, $thumb = false, $crop = false)
    {
        ...
        if ($thumb !== false)
        {
            $params['buzzcrud']['thumb'] = $thumb;
        }
        if ($crop)
        {
            $params['buzzcrud']['crop'] = $crop;
        }
        if (buzzcrud_config::$dynamic_session)
        {
            $params['buzzcrud']['sess_name'] = session_name();
        }
        return buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$ajax_uri . '?' . http_build_query($params);
    }
    protected function html_safe($text)
    {
        return htmlspecialchars((string )$text, ENT_QUOTES, buzzcrud_config::$mbencoding);
    }
    ...
    protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
    {
        $header = 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/' . ($html ? 'html' : 'plain') . '; charset=UTF-8' . "\r\n" .
            'From: ' . buzzcrud_config::$email_from_name . ' <' . buzzcrud_config::$email_from . ">\r\n";
        if ($cc)
    ...
    protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false)
    {
        ...
        if ($field == $order && $this->is_sortable)
            $column_class[] = 'buzzcrud-current';
        if ($is_sum)
            $column_class[] = 'buzzcrud-sum';
        if ($row_color)
        {
    ...
    {
        if (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/' . $this->language . '.ini'))
            $this->lang_arr = parse_ini_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/' . $this->language . '.ini');
        elseif (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/en.ini'))
            $this->lang_arr = parse_ini_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/en.ini');
        if ($this->set_lang)
        {
    ...
    {
        if (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/' . buzzcrud_config::$language . '.ini'))
            return parse_ini_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/' . buzzcrud_config::$language . '.ini');
        elseif (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/en.ini'))
            $this->lang_arr = parse_ini_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/en.ini');
    }
    protected function _get_theme_config()
    { // loads theme configuration from ini file
        if (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$themes_path . '/' . $this->theme . '/buzzcrud.ini'))
            $this->theme_config = parse_ini_file(buzzcrud_PATH . '/' . buzzcrud_config::$themes_path . '/' . $this->theme . '/buzzcrud.ini');
        else
            self::error('buzzcrud.ini does not exist in your theme folder');
    }
    protected function lang($text = '')
    {
        $langtext = mb_convert_case($text, MB_CASE_LOWER, buzzcrud_config::$mbencoding);
        return htmlspecialchars((isset($this->lang_arr[$langtext]) ? $this->lang_arr[$langtext] : $text), ENT_QUOTES,
            buzzcrud_config::$mbencoding);
    }
    protected function theme_config($text = '')
    {
        $text = mb_convert_case($text, MB_CASE_LOWER, buzzcrud_config::$mbencoding);
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, buzzcrud_config::
            $mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        if (self::$css_loaded)
        {
            self::error('buzzcrud\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }

        self::$css_loaded = true;
        if (buzzcrud_config::$load_bootstrap)
        {
            $out .= '<link href="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri .
                '/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />';
        }
        if (buzzcrud_config::$load_jquery_ui)
            $out .= '<link href="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri .
                '/jquery-ui/jquery-ui.min.css" rel="stylesheet" type="text/css" />';
        if (buzzcrud_config::$load_jcrop)
            $out .= '<link href="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri .
                '/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
        $out .= '<link href="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri .
            '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . buzzcrud_config::$scripts_url .
            '/' . buzzcrud_config::$themes_uri . '/' . buzzcrud_config::$theme . '/buzzcrud.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        {
            $language = buzzcrud_config::$language;
        ...
        if (self::$js_loaded)
        {
            self::error('buzzcrud\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }
        self::$js_loaded = true;
        if (buzzcrud_config::$load_jquery)
            $out .= '<script src="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri . '/jquery.min.js"></script>';
        if (buzzcrud_config::$jquery_no_conflict)
        {
            $out .= '
        ...
        if (buzzcrud_config::$load_jquery_ui)
            $out .= '<script src="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri .
        ...
        if (buzzcrud_config::$editor_url)
            $out .= '<script src="' . buzzcrud_config::$editor_url . '"></script>';
        if (buzzcrud_config::$load_googlemap)
            $out .= '<script src="//maps.google.com/maps/api/js?sensor=false&language=' . $language . '"></script>';
        $out .= '<script src="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$plugins_uri . '/buzzcrud.js"></script>';

        $config = array(
            'url' => buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$ajax_uri,
            'editor_url' => buzzcrud_config::$editor_url,
            'editor_init_url' => buzzcrud_config::$editor_init_url,
            'force_editor' => buzzcrud_config::$force_editor,
            'date_first_day' => buzzcrud_config::$date_first_day,
            'date_format' => buzzcrud_config::$date_format,
            'time_format' => buzzcrud_config::$time_format,
            'lang' => $lang_arr,
            'rtl' => buzzcrud_config::$is_rtl ? 1 : 0);
        $out .= '
            <script type="text/javascript">
            <!--
            
           	var buzzcrud_config = ' . json_encode($config) . ';
                            
            -->
            </script>';
        if ($language != 'en')
        {
            if (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
                $out .= '<script src="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$lang_uri .
                    '/datepicker/jquery.ui.datepicker-' . $language . '.js"></script>';
            if (is_file(buzzcrud_PATH . '/' . buzzcrud_config::$lang_path . '/timepicker/jquery-ui-timepicker-' . $language . '.js'))
                $out .= '<script src="' . buzzcrud_config::$scripts_url . '/' . buzzcrud_config::$lang_uri .
    ...
    protected function get_limit_list($limit = 20, $buttons = false)
    {
        ...
        if ($buttons)
        {
            $out .= $this->open_tag(array(
                'tag' => 'div',
                'class' => 'btn-group buzzcrud-limit-buttons',
            ...
            foreach ($this->limit_list as $limts)
            {
                ...
                $out .= $this->open_tag(array(
                    ...
                    'data-limit' => $limts), 'buzzcrud-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
            }
        ...
        else
        {
            $out .= $this->open_tag(array(
                ...
                'name' => 'limit'), 'buzzcrud-actionlist buzzcrud-data');
            foreach ($this->limit_list as $limts)
            {
    ...
    {
        if (!buzzcrud_config::$performance_mode)
        {
    ...
    {
        if (!is_array($value) && !buzzcrud_config::$performance_mode && $value)
        {
    ...
    protected function send_http_request($url, $data, $method, $return_result = false)
    {
        ...
        {
            case 'get':
                $ch = curl_init($path . ((mb_strpos($path, '?', 0, buzzcrud_config::$mbencoding) === false) ? '?' : '&') . $data);
                break;
            case 'post':
                $ch = curl_init($path);
        ...
        if (buzzcrud_config::$use_browser_info)
        {
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'buzzcrud-tabs')) . $this->
            ...
            if ($this->nested_rendered && buzzcrud_config::$nested_in_tab)
            {
    ...
    protected function get_table_tooltip()
    {
        ...
        if ($this->table_tooltip)
        {
            ...
            $out .= $this->open_tag(array(
                'tag' => 'a',
                'href' => 'javascript:;',
                'class' => 'buzzcrud-tooltip buzzcrud-button-link',
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            $attr = array('class' => 'buzzcrud-search-toggle', 'href' => 'javascript:;');
            if ($this->search or buzzcrud_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('a', $this->theme_config('search_open'), $attr);
            $out .= $this->lang('search') . $this->close_tag('a');
            $attr = array('class' => 'buzzcrud-search');
            if (!$this->search && !buzzcrud_config::$search_opened)
            {
                $attr['style'] = 'display:none;';
            }
            $out .= $this->open_tag('span', $this->theme_config('search_container'), $attr);
            if (buzzcrud_config::$search_all)
            {
            ...
            $attr = array('class' => 'buzzcrud-data', 'name' => 'column');
            $out .= $this->open_tag('select', 'buzzcrud-columns-select ' . $this->theme_config('search_fieldlist'), $attr);
            //$out .= $this->open_tag('option', '', array('value' => '')) . $this->lang('all_fields') . $this->close_tag('option');
            $out .= implode('', $optlist);
            $out .= $this->close_tag('select');
            $attr = array(
                'class' => 'buzzcrud-action',
                'href' => 'javascript:;',
                'data-search' => 1);
    ...
    {
        $attr_preset = array('class' => 'buzzcrud-searchdata', 'name' => 'phrase');
        if ($field == $this->column)
        {
            $class = 'buzzcrud-search-active';
        }
        else
        {
            $class = '';
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'timestamp':
                if (!isset($fieldlist['date']) or $field == $this->column)
                {
                    ...
                    $attr_range = array(
                        'class' => 'buzzcrud-daterange buzzcrud-searchdata ' . $this->theme_config('search_range'),
                        'name' => 'range',
                        'data-fieldtype' => 'date');
                    if ($field != $this->column)
                    {
                    ...
                    $fieldlist['date'] .= $this->open_tag('option', '', array('value' => '')) . $this->lang('choose_range') . $this->
                        close_tag('option');
                    if (buzzcrud_config::$available_date_ranges)
                    {
                        foreach (buzzcrud_config::$available_date_ranges as $range)
                        {
                    ...
                    $fieldlist['date'] .= $this->single_tag('input', 'buzzcrud-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                        $attr);
                    $attr['name'] = 'phrase][to';
                    $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                    $fieldlist['date'] .= $this->single_tag('input', 'buzzcrud-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                        $attr);
                }
                break;
            case 'select':
    ...
    {
        $out = '';
        $out .= $this->open_tag($row, 'buzzcrud-th');
        if ($this->is_numbers)
        {
            $out .= $this->open_tag($item, 'buzzcrud-num') . '&#35;' . $this->close_tag($item);
        }
        ...
        {
            $out .= $this->open_tag($item, 'buzzcrud-actions') . '&nbsp;' . $this->close_tag($item);
        }
        foreach ($this->columns as $field => $fitem)
        {
            ...
            if ($this->is_sortable)
            {
                ...
                {
                    $class .= ' buzzcrud-primary';
                }
                if ($this->order_column == $field)
                {
                    $class .= ' buzzcrud-current buzzcrud-' . $this->order_direct;
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                $out .= $this->open_tag($row_tag, 'buzzcrud-row buzzcrud-row-' . $i);
                if ($this->is_numbers)
                {
                    $out .= $this->open_tag($item, 'buzzcrud-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                        $this->start + 1) . $this->close_tag($item);
                }
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
                {
                    $out .= $this->open_tag($item, 'buzzcrud-actions', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                    $out .= $this->open_tag($item, 'buzzcrud-actions' . (buzzcrud_config::$fixed_action_buttons ? ' buzzcrud-fix' : ''), $this->
        ...
        else
        {
            ...
            $out .= $this->open_tag($row_tag, 'buzzcrud-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .
    ...
    protected function render_grid_footer($row = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        {
            $out .= $this->open_tag($row, 'buzzcrud-tf');
            if ($this->is_numbers)
            {
                $out .= $this->open_tag($item, 'buzzcrud-num') . '&Sigma;' . $this->close_tag($item);
            }
    ...
    protected function render_benchmark($tag = array('tag' => 'span'))
    {
        if ($this->benchmark)
        {
            return $this->open_tag($tag, 'buzzcrud-benchmark') . $this->benchmark_end() . $this->close_tag($tag);
        }
        return '';
    }
    protected function render_control_fields()
    {
        ...
        $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : buzzcrud_config::$limit)));
    ...
    protected function render_message()
    {
        ...
        if ($this->message)
        {
            $tag = array(
                'tag' => 'input',
                'type' => 'hidden',
                'class' => 'buzzcrud-callback-message',
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            $out .= $this->open_tag($tag, 'buzzcrud-action');
            if ($icon && !$this->is_rtl)
            {
    ...
    protected function csv_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('export_csv', 'csv', '', $class . ' buzzcrud-in-new-window', $icon);
        }
    }
    protected function print_button($class = '', $icon = '')
    {
        ...
        {
            return $this->render_button('print', 'print', '', $class . ' buzzcrud-in-new-window', $icon);
        }
    }
    ...
    protected function get_image_folder($field)
    {
        ...
        {
            $path = $this->check_folder(buzzcrud_config::$upload_folder_def, 'get_image_folder');
        }
    ...
    {
        $range = array();
        $time = time() /* + 3600 * buzzcrud_config::$local_time_correction*/;
        $week_day = date('w', $time) /* + buzzcrud_config::$date_first_day*/;
        switch ($name)
        {
            default:
            case 'today':
            ...
            case 'this_week_today':
                if ($week_day >= buzzcrud_config::$date_first_day)
                {
                    $offset1 = $week_day - buzzcrud_config::$date_first_day;
                }
                else
                {
                    $offset1 = 7 - (buzzcrud_config::$date_first_day - $week_day);
                }
            ...
            case 'this_week_full':
                ...
                $offset2 = 6 - $week_day + buzzcrud_config::$date_first_day;
                $range['from'] = mktime(0, 0, 0, date('n', $time), date('j', $time) - $offset1, date('Y', $time));
    ...
    {
        if ($time)
            return $utc ? gmdate(buzzcrud_config::$php_date_format, $time) : date(buzzcrud_config::$php_date_format, $time);
        else
            return '';
    }
    ...
    {
        if ($time)
            return $utc ? gmdate(buzzcrud_config::$php_date_format . ' ' . buzzcrud_config::$php_time_format, $time) : date(buzzcrud_config::
                $php_date_format . ' ' . buzzcrud_config::$php_time_format, $time);
        else
            return '';
    }
    ...
    {
        if ($time)
            return $utc ? gmdate(buzzcrud_config::$php_time_format, $time) : date(buzzcrud_config::$php_time_format, $time);
        else
            return '';
    }
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            if ($to_show)
                $out .= '<span class="buzzcrud-toggle-show buzzcrud-toggle-down"><i class="' . $this->theme_config('slide_down_icon') .
                    '"></i></span>';
            else
                $out .= '<span class="buzzcrud-toggle-show buzzcrud-toggle-up"><i class="' . $this->theme_config('slide_up_icon') .
                    '"></i></span>';
            $out .= $this->close_tag($tag);
        }
        return $out;
    ...
    {
        if (!buzzcrud_config::$alt_encription_key)
        {
        ...
        $key = substr(buzzcrud_config::$alt_encription_key, 0, $ks);
    ...
    {
        $_SESSION['buzzcrud_session'] = $data;
    }
    public static function export_session()
    {
        return $_SESSION['buzzcrud_session'];
    }
    ...
    {
        $path = buzzcrud_PATH . '/core/' . $name . '.php';
...
class buzzcrud_postdata
{
    private $buzzcrud = null;
    private $postdata = array();
    public function __construct($postdata, $buzzcrud)
    {
        $this->buzzcrud = $buzzcrud;
        $this->postdata = $postdata;
        unset($postdata);
    }
    public function set($name, $value)
    {
        $fdata = $this->buzzcrud->_parse_field_names($name, 'buzzcrud_postdata');
        foreach ($fdata as $key => $fitem)
        {
            $this->postdata[$key] = $value;
        }
        $this->buzzcrud->unlock_field($name);
        return $this;
    }
    public function get($name)
    {
        ...
        return new buzzcrud_postdata_item($value);
    }
